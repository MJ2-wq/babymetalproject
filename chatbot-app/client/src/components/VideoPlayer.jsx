...
/**
 * VideoPlayer - Social media style video player with custom controls.
 * Features: play/pause, progress bar, volume, fullscreen, time display.
 * Auto-plays when visible, pauses when off-screen.
 */
export default function VideoPlayer({ src, poster, onOpenViewer }) {
  ...
  return (
    <div className={`vp-container ${playing ? "playing" : ""}`} ref={containerRef}>
      <video
        ref={videoRef}
        className="vp-video"
        src={src}
      ...
      {/* Bottom controls */}
      <div className={`vp-controls ${playing ? "auto-hide" : ""}`}>
        {/* Progress bar */}
        <div className="vp-progress" ref={progressRef} onClick={handleProgressClick}>
        ...
        <div className="vp-controls-row">
          <div className="vp-left">
            ...
            <div className="vp-volume-wrap" onMouseEnter={() => setShowVolume(true)} onMouseLeave={() => setShowVolume(false)}>
              <button className="vp-btn" onClick={toggleMute}>
                {muted || volume === 0 ? (
                  ...
                  <svg viewBox="0 0 24 24" width="20" height="20" fill="white"><path d="M18.5 12c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM5 9v6h4l5 5V4L9 9H5z"/></svg>
                ) : (
